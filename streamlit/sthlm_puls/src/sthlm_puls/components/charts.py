...
def chart_style(fig: plt.Figure, ax, title: str = "") -> None:
...
def plot_events_weather(df_merged: pd.DataFrame) -> plt.Figure:
    ...
    chart_style(fig, ax, "This week's cultural events in Stockholm — pick the right day")
    return fig


def plot_events_weekday(df: pd.DataFrame) -> plt.Figure:
    ...
    chart_style(fig, ax, "Stockholm is a weekend city – saturday dominates")
    return fig
...
def plot_segment_over_time(df: pd.DataFrame) -> plt.Figure:
    ...
    chart_style(fig, ax, "Stockholm's cultural calendar — events by segment over the year")
    return fig